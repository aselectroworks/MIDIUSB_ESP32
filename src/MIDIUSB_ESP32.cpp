...
#define TUSB_DESCRIPTOR_ITF_MIDI_LEN \
    (TUD_MIDI_DESC_HEAD_LEN + TUD_MIDI_DESC_JACK_LEN * USB_MIDI_NUM_CABLES + TUD_MIDI_DESC_EP_LEN(USB_MIDI_NUM_CABLES) * 2)
...
{
    uint8_t descriptor[TUSB_DESCRIPTOR_ITF_MIDI_LEN] = {TUD_MIDI_DESC_HEAD(*itf, 4, USB_MIDI_NUM_CABLES),
...
__attribute__((weak)) void usbEventCallback(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    if (event_base == ARDUINO_USB_EVENTS) {
        ...
        switch (event_id) {
            default:
                HWSerial.printf("MIDI EVENT:  ID=%ld, DATA=%lu\r\n", event_id, (uint32_t)data);
                break;
        }
    }
}
